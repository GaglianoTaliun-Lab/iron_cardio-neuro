...
arg = commandArgs(T); ref.prefix = arg[1]; loc.file = arg[2]; info.file = arg[3]; phenos = unlist(strsplit(arg[4],";")); out.fname = arg[5]
...
input = process.input(info.file, NULL, ref.prefix, phenos)
...
for (i in 1:n.loc) {
  ...
  if (!is.null(locus)) {
    ...
    loc.out = run.univ.bivar(locus, univ.thresh=0.05/2495)
